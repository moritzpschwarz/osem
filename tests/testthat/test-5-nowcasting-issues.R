...
test_that("Checking nowcasting issue when one independent variable is not available for a period",{
  ...
  expect_message(f1 <- forecast_model(model, quiet = FALSE, exog_fill_method = "auto", plot = FALSE), regexp = "No exogenous values")
  expect_message(f2 <- forecast_model(model, quiet = FALSE, exog_fill_method = "AR", plot = FALSE), regexp = "No exogenous values")
  expect_message(f3 <- forecast_model(model, quiet = FALSE, exog_fill_method = "last", plot = FALSE), regexp = "No exogenous values")
  expect_message(f3 <- forecast_model(model, quiet = FALSE, exog_fill_method = "last", plot = FALSE), regexp = "Latest Exogenous Data is not available")
