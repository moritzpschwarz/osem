...
# Umwelt
env <- c("env_ac_aibrid_r2", # Übergang zu Emissionsinventaren
         "t2020_35", # ESD
...
tibble(orig = gdp_vars_to_estimate, 
...
gdp_vars_to_estimate_gets <- dictionary %>% filter(new %in% gets_result$aux$mXnames) %>% pull(orig)
...
estimate_and_predict <- function(data, n_ahead){
  ...
  newdata <- data %>% as_tibble() %>% slice(rep(n(),n_ahead)) %>% as.list
