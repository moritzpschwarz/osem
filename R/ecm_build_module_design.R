build_module_design <- function(clean_data,
  ...
  # Return the exact transformation applied during data preparation. The
  # use_logs argument alone cannot distinguish log() from asinh().
  ...
  # Collect metadata alongside the numerical estimation design --------------
  ...
  # Build a fully differenced equation --------------------------------------
  if (model_form == "diff") {
    y.name <- paste0(
      ifelse(log_opts %in% c("both", "y"), "D.ln.", "D."),
      dep_var_basename
