    ...
    if(!is.null(inputdata_directory)){
      ...
      if (is.character(inputdata_directory)) {
        if (file.exists(inputdata_directory) & !dir.exists(inputdata_directory)) {
          stop("The variable 'inputdata_directory' must be a character path to a directory, not to a file. To use files, use the argument 'input'.")
        }
      ...
      warning("'inputdata_directory' is deprecated. Use 'input' instead. Functionality is being retained for now but this argument will be removed in the future.")
    }
  }
