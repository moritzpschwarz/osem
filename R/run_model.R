...
run_model <- function(specification,
  ...
  if (any(duplicated(dictionary$model_varname))) {
    stop("Dictionary cannot contain duplicated values for 'model_varname'.")
  }

  if (any(grepl("\\-|\\+|\\*|\\/|\\^", dictionary$model_varname))) {
    stop("The 'model_varname' column in the Dictionary cannot contain any of the following characters: + - / * ^")
  }
  if (any(grepl("\\-|\\+|\\*|\\/|\\^", specification$dependent))) {
    stop("The 'dependent' column in the specification cannot contain any of the following characters: + - / * ^")
  }
  if (any(grepl("\\-|\\*|\\/|\\^", specification$independent[specification$type == "n"]))) {
    stop("The 'independent' column of estimated equations (type = 'n') in the specification cannot contain any of the following characters: - / * ^.\nThis is only possible for identities (type = 'd').")
  }


  if (!is.null(save_to_disk)) {
    if (!is.character(save_to_disk)) {
      stop("'save_to_disk' must be a path to a file as a character. For example 'data.csv'.")
    }
    if (is.character(save_to_disk) & identical(strsplit(basename(save_to_disk), split = "\\.")[[1]][-1], character(0))) {
      stop("'save_to_disk' must be a path to a file. No file ending detected. Currently supporting RDS, rds, Rds, csv, xls, xlsx.")
    }
  }
  ...
  if (!is.null(keep) & !is.character(keep)) {
    stop("The argument 'keep' must be a character vector.")
  }
  # raise an error when the keep argument is specified but gets_selection is FALSE
  if (!is.null(keep) & !gets_selection) {
    stop("The argument 'keep' can only be specified when 'gets_selection' is TRUE.")
  }
  if (!is.null(keep) & length(keep) > 1) {
    stop("The argument 'keep' can only be a character - not a vector.")
  }

  if (!is.numeric(saturation.tpval) & !is.null(saturation.tpval)) {
    stop("'saturation.tpval' must be either NULL or numeric between 0 and 1.")
  }
  if (is.numeric(saturation.tpval) & (saturation.tpval > 1 | saturation.tpval < 0)) {
    stop("'saturation.tpval' must be either NULL or numeric between 0 and 1.")
  }
  if (!is.numeric(selection.tpval) & !is.null(selection.tpval)) {
    stop("'selection.tpval' must be either NULL or numeric between 0 and 1.")
  }
  if (is.numeric(selection.tpval) & (selection.tpval > 1 | selection.tpval < 0)) {
    stop("'selection.tpval' must be either NULL or numeric between 0 and 1.")
  }

  # Checking log specifications
  if (length(use_logs) > 1 | !is.character(use_logs) | !all(use_logs %in% c("y", "x", "both", "none"))) {
    stop("The argument 'use_logs' must be a character vector and can only be one of 'both', 'y', 'x', or 'none'.")
  }
  ...
  if (is.null(ardl_or_ecm) | (!identical(ardl_or_ecm, "ardl") & !identical(ardl_or_ecm, "ecm"))) {
    stop("The argument 'ardl_or_ecm' must be a character vector and can only be one of 'ardl' or 'ecm'.")
  }
  ...
  for (i in module_order$order) {
    ...
    if (!quiet) {
      ...
      if (module_order$type[i] == "n") {
        cat(paste0("Estimating ", module_order$dependent[i], " = ", module_order$independent[i]), "\n")
      }
      if (module_order$type[i] == "d") {
        cat(paste0("Constructing ", module_order$dependent[i], " = ", module_order$independent[i]), "\n")
      }
    }
