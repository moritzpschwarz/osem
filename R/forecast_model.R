...
forecast_model <- function(model, exog_predictions = NULL, n.ahead = 10, plot = TRUE){
  ...
  for(i in 1:2){
    ...
    current_spec <- model$module_order_eurostatvars %>%
      ...
      drop_na(index) %>%
      select(index, dependent_eu, independent_eu, ind_vars) %>%
    ...
    if(!all(current_spec$independent_eu %in% names(exog_df_ready)) & !is.na(current_spec$ind_vars)){

      missing_vars <- current_spec$ind_vars[!current_spec$ind_vars %in% names(exog_df_ready)]

      for(mvar in missing_vars){
        # mvar = "co"

        model$module_order_eurostatvars %>%
          filter(tolower(dependent) == mvar) %>%
