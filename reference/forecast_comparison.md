...
back to the level of the variable. Also, the AR model is estimated on
the same (sub)sample as the OSEM model. Since OSEM modules may contain
explanatory variables whose availability restricts the sample, we ensure
that the AR model is estimated on the same data as OSEM to ensure a fair
comparison.
